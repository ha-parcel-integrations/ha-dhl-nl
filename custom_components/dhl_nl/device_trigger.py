"""Device triggers for the DHL Package Tracker integration.

Surfaces the parcel events the coordinator fires on the HA event bus as
no-code automation triggers, so users can pick "a parcel was registered"
from the automation UI instead of hand-writing an event trigger. Each
trigger filters on the account's ``device_id`` (attached to every event)
so multi-account setups only fire for the selected device.
"""
...
async def async_get_triggers(
    ...
    """Return the list of parcel triggers for a DHL device."""
    return [
        {
...
async def async_attach_trigger(
    ...
    The event trigger filters on the ``device_id`` carried in each parcel
    event, so the automation only fires for the selected account.
    """
