...
function releaseNotes(changes, helpWanted, repo) {
  const sections = [["New features", changes.filter((change) => change.type === "feat")], ["Bug fixes", changes.filter((change) => change.type === "fix")]];
  const lines = [];
...
async function changesSinceRelease(github, context, tag, head) {
  const { data } = await github.rest.repos.compareCommits({ owner: context.repo.owner, repo: context.repo.repo, base: tag, head });
  if (data.total_commits > data.commits.length) throw new Error("More than 250 commits since the last release; split the release before continuing.");
  return data.commits.flatMap((commit) => {
    const match = RELEASE_SUBJECT.exec(commit.commit.message.split("\n", 1)[0]);
    return match ? [{ type: match[1], breaking: Boolean(match[2]), description: match[3] }] : [];
  });
}
...
module.exports = async ({ github, context, core }) => {
  ...
  const changes = await changesSinceRelease(github, context, tag, process.env.HEAD_SHA || context.sha);
  const requested = process.env.RELEASE_VERSION;
  const candidate = changes.length ? bumpedVersion(tag, changes) : null;
  if (requested) {
    if (!PRERELEASE_VERSION.test(requested)) throw new Error("Pre-release version must use X.Y.ZbN, for example 2.9.0b1.");
    if (!candidate) throw new Error("A pre-release needs at least one feat: or fix: commit since the stable release.");
    if (!requested.startsWith(`${candidate}b`)) throw new Error(`Pre-release ${requested} must be based on the calculated next version ${candidate}.`);
  } else if (!candidate) {
  ...
  const { data: issues } = await github.rest.issues.listForRepo({ owner: context.repo.owner, repo: context.repo.repo, state: "open", labels: "help wanted", per_page: 100 });
