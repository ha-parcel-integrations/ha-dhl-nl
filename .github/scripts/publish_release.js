...
module.exports = async ({ github, context, core }) => {
  ...
  if (!match) { core.notice("The validated main commit is not a release bump."); return; }
  ...
  if (!releasePr) { core.notice("The version bump was not created by the release manager."); return; }
  try {
  ...
  await github.rest.git.createRef({ owner: context.repo.owner, repo: context.repo.repo, ref: `refs/tags/${version}`, sha });
  await github.rest.repos.createRelease({
    ...
    name: version,
    body: releasePr.body.replace(marker, "").trim(),
